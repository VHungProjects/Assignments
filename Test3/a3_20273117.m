...
% Function for CISC271, Winter 2024, Assignment #3
    ...
    %% TEST
    disp("TEST HERE")
    yk3 = kmeans(ZSTD, 3, 'start', [1 1 ; 0 -2 ; 3 1]);
    correct = sum((yk3 == lvec) == 1);
    incorrect = 178 - correct;
    disp(incorrect);

    differences_count = sum(lvec ~= yk3);

    disp(['Number of differences between labels and cluster indexes: ', num2str(differences_count)]);
end
